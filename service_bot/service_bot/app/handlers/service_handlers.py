...
from app.keyboards.service_keyboards import (
    ...
    get_service_status_change_keyboard
)
...
@router.callback_query(F.data.startswith("service_change_status_"))
async def change_service_order_status(callback: CallbackQuery):
    """Обработчик нажатия на кнопку изменения статуса заказа СТО"""
    ...
    if statuses_response.get("status") == 200 and "statuses" in statuses_response.get("data", {}):
        ...
        status_response = await api_service.get_order_status(order_number, order_type="ЗаказСТО")
        ...
        if status_response.get("status") == 200:
            ...
            keyboard = get_service_status_change_keyboard(order_number, statuses)
            
            await callback.message.answer(
                f"🔄 <b>Изменение статуса заказа СТО #{order_number}</b>\n\n"
...
@router.callback_query(F.data.startswith("service_set_status_"))
async def set_service_order_status(callback: CallbackQuery):
    """Обработчик нажатия на кнопку выбора нового статуса заказа СТО"""
    parts = callback.data.split("_")
    order_number = parts[3]
    new_status = parts[4]
    
    response = await api_service.update_order_status(order_number, new_status, order_type="ЗаказСТО")
    ...
    if response.get("status") == 200:
        ...
        if statuses_response.get("status") == 200 and "statuses" in statuses_response.get("data", {}):
            ...
            await callback.message.answer(
                f"✅ Статус заказа СТО #{order_number} успешно изменен на <b>{status_name}</b>\n"
                f"🕒 Время обновления: {updated_at}"
            )
            
            new_callback_data = f"service_order_{order_number}"
            callback.data = new_callback_data
            await show_service_order_details(callback)
        else:
            await callback.answer("✅ Статус успешно обновлен")
            await callback.message.answer(f"✅ Статус заказа СТО #{order_number} успешно обновлен на {updated_status}")
    else:
        error_msg = response.get("data", {}).get("message", "Неизвестная ошибка")
        await callback.answer("❌ Ошибка при обновлении статуса", show_alert=True)
        await callback.message.answer(f"❌ Ошибка при обновлении статуса заказа СТО: {error_msg}")
